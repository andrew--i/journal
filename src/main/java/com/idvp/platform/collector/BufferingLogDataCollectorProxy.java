 ...
 */
package com.idvp.platform.collector;

import com.idvp.platform.configuration.LogCollectorConfiguration;
import com.idvp.platform.io.Stoppable;
...
public class BufferingLogDataCollectorProxy implements LogDataCollector, Stoppable {
  ...
  public BufferingLogDataCollectorProxy(LogDataCollector delegate, final long sleepTime, LogCollectorConfiguration configuration) {
    super();
    this.delegate = delegate;
    proxyLogDataCollector = new ProxyLogDataCollector();
    Runnable r = () -> {
      while (!stop) {
        if (configuration.getBoolean(LogCollectorConfiguration.TAILING_PANEL_PLAY)) {
