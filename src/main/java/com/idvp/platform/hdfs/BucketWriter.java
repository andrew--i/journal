...
public class BucketWriter {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(BucketWriter.class.getName());
     ...
     * @throws InterruptedException
     */
    public synchronized void append(final byte[] data)
            throws IOException, InterruptedException {
        checkAndThrowInterruptedException();
        ...
        try {
            callWithTimeout((CallRunner<Void>) () -> {
                writer.append(data); // could block
                return null;
            });
        } catch (IOException e) {
